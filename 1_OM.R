...
dir.create(path = "input/whg4", recursive = TRUE)
dir.create(path = "output/runs/whg4", recursive = TRUE)
...
set.seed(1)
uncertainty <- SAM_uncertainty(fit = fit, n = n, idx_cov=TRUE,catch_est=TRUE, print_screen = FALSE)
### add noise to stock
...
plot(catch_n,probs = c(0.05, 0.25, 0.5, 0.75, 0.95))

### ------------------------------------------------------------------------ ###
### check MCMC approach ####
### ------------------------------------------------------------------------ ###
if(FALSE){ library(tmbstan) # cran package
# 
...
tiff(paste0("output/",n,"MCMC_stk.tiff"), bg="white",  res=200, width = 1200, height = 1100)
par(mar=c(4,4,4,1))
...
par(mar=c(4,4,4,2))
plot(residuals(sr_), probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
dev.off()
...
plot(sr_res10, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
...
par(mar=c(4,4,4,2))
plot(residuals(sr1_0), probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
dev.off()
...
par(mar=c(4,4,4,2))
plot(res2, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
dev.off()
...
par(mar=c(4,4,4,2))
plot(res3, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
dev.off()
...
par(mar=c(4,4,4,2))
hist(residuals(sr_)[,36:55,,,,],col=rgb(1,0,0,0.2), xlab="",ylim=c(0,6000), main="histogram residuals 1983 series")
legend("topright",legend=c("without level change", "with level change"),bty="n", col=c(rgb(1,0,0,0.2),rgb(0,0,1,0.2) ), pch=15)
hist(residuals(sr_dip)[,36:55,,,,],col=rgb(0,0,1,0.2), add=T)
dev.off()

tiff(paste0("output/",n,"res2002_no_hist.tiff"), bg="white",  res=200, width = 900, height =1100)
par(mar=c(4,4,4,2))
hist(residuals(sr1_0)[,17:36,,,,],col=rgb(1,0,0,0.2), xlab="",ylim=c(0,6000), main="histogram residuals 2002 series")
legend("topright",legend=c("without level change", "with level change"),bty="n", col=c(rgb(1,0,0,0.2),rgb(0,0,1,0.2) ), pch=15)
hist(residuals(sr1_0_dip)[,17:36,,,,],col=rgb(0,0,1,0.2), add=T)
dev.off()
...
par(mar=c(4,4,4,2))
plot(proc_res, probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
dev.off()
...
par(mar=c(4,4,4,2))
plot(catch_res_l, xlab="Year", probs = c(0.05, 0.25, 0.5, 0.75, 0.95))
dev.off()
...
#ssb(sr)<-catch_res_l  #load catch residuals for implementation error
