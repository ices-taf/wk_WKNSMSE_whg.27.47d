...
load(file = paste0("input/whg4/base_image/image_",om_opt,"_1000.RData"))
...
refpts_mse <- list(Btrigger = 166708,
                   Blim = 119970,
				           Bpa = 166708,
        				   Fpa = 0.33,
				           Ftrgt = 0.172)  # vary for different reference points
				   
### some specifications for short term forecast with SAM
whg4_stf_def <- list(fwd_yrs_average = -3:0,
...
### implementation error model (banking and borrowing)
 iem <- FLiem(method = iem_WKNSMSE, 
              args = list(BB = FALSE, imp.error=TRUE, imp.res=catch_res_l))  # extra implementation error
...
ctrl_obj <- mpCtrl(list(
  ...
  ctrl.phcr = mseCtrl(method = phcr_WKNSMSE,
                      args = refpts_mse),
  ctrl.hcr = mseCtrl(method = hcr_WKNSME, args = list(option = "A")),  # here option A is default can be cahged in run script
